...
const movieIds = [2, 50341, 5462, 3034, 50622, 2345, 9417, 612, 98089];
        ...
        .then(response => {
            ...
            */
            document.getElementById('movieListView').innerHTML +=
                `
                <div class="movieCard">
                <img src="https://image.tmdb.org/t/p/original${response.poster_path} " alt="movie poster" class="movieImg">
                
                <div class="movieCarte">
                <p class="movieTitle">${response.name}</p> 
                
                <span class="movieRate"><small>${response.vote_average}</small></span>
                </div>
                `;
        })
